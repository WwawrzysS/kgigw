...
function dataLoadingHtml() {
  return '<div class="row"><small>\u0141adowanie danych\u2026</small></div>';
}
  ...
  return `
    <div class="row">
      <div>
        ...
        <small>Sprawd\u017a po\u0142\u0105czenie i spr\u00f3buj ponownie.</small>
      </div>
      <div class="row-actions">
        <button class="small-button" type="button" onclick="startSupabaseDataLoad({ reason: 'retry-button', force: true })">Pon\u00f3w pobieranie danych</button>
      </div>
    </div>
  `;
}
...
function renderDashboardDataLoadPlaceholder(error = false) {
  const message = error
    ? "Nie uda\u0142o si\u0119 pobra\u0107 danych. Sprawd\u017a po\u0142\u0105czenie i spr\u00f3buj ponownie."
    : "\u0141adowanie danych\u2026";
  const html = error ? dataErrorHtml() : dataLoadingHtml();
  [
    elements.recentMoney,
    elements.upcomingEvents,
    elements.unpaidInvoicesList
  ].forEach((element) => setElementLoadingState(element, html));
  [
    elements.dashboardTodaySummary,
    elements.dashboardFocus,
    elements.cashBalance,
    elements.unpaidInvoicesSummary,
    elements.lateFees,
    elements.rentalCount
  ].forEach((element) => {
...
function renderStorageInfo() {
  ...
  if (storageLoadStatus === "loading" && !supabaseDataReady) {
    elements.storageTexts.forEach((item) => {
      item.textContent = "Sprawdzanie miejsca na pliki\u2026";
    });
  ...
  if (storageLoadStatus === "error" && !state.docs?.length) {
    elements.storageTexts.forEach((item) => {
      item.textContent = "Nie uda\u0142o si\u0119 sprawdzi\u0107 miejsca na pliki.";
    });
    elements.storageBars.forEach((item) => {
      item.style.width = "0%";
